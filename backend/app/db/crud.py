...
def delete_model_record_by_task_id(db: Session, task_id: str) -> int:
    """根据 task_id 删除 ModelRecord 表中的记录"""
    num_deleted = db.query(db_models.ModelRecord).filter(
        db_models.ModelRecord.task_id == task_id
    ).delete(synchronize_session=False)
    # 注意：commit 由调用方的 API 路由统一处理
    return num_deleted

def delete_task_by_task_id(db: Session, task_id: str) -> int:
    """根据 task_id 删除 TaskProgress 表中的记录"""
    num_deleted = db.query(db_models.TaskProgress).filter(
        db_models.TaskProgress.task_id == task_id
    ).delete(synchronize_session=False)
    # 注意：commit 由调用方的 API 路由统一处理
    return num_deleted
